...
const Login = () => {
  ...
  return <>
    <div className="loginContainer" style={{display: 'flex', flexDirection: 'column', justifyContent:'center', alignItems:'center', height: '100vh'}}>
      ...
      <p style={{fontSize:'25px',color:'white'}}>{resultMessage}</p>
      <Link to='/' onClick={()=>setPressed(false)} style={{marginTop:'20px',fontSize:'30px',color:'white',padding:'10px',backgroundColor:'coral',borderRadius:'10px',textDecoration:'none'}}>Try to login again</Link>
      </div>) : <p></p>}
    </div>
  </>
}
